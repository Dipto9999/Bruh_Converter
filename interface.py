...
def initScreen() :
    nameButton = tk.Button(pane, text = 'Hi. What is your name?', 
        bg = general_button_bg, fg = 'black', 
        bd = 5, borderwidth = 2, 
        command = lambda : startConversion(str(input_field.get())))
    ...
    input_field = tk.Entry(pane, width = 30, 
        bg = general_entry_bg, fg = 'black', 
        bd = 5, borderwidth = 2)
...
def startConversion(name) :
    ...
    prepare_conversion = tk.Label(pane, text = f'Hello {name}. I have something to tell you. ', 
        bg = general_label_bg, fg = 'black', 
        anchor = tk.E, bd = 5, 
        borderwidth = 2)
    prepare_conversion.grid(row = 2, column = 0, 
        padx = 10, pady = 10)
    ...
    if (message_to_console != 'Cannot BRUH-ify. Your name is too short.') :
        ...
        prepare_reduced = tk.Button(pane, text = 'Do you want to BRUH-ify again? ', 
            bg = general_button_bg, fg = 'black', 
            bd = 5, borderwidth = 2, 
            command = lambda : continueConversion(message_to_console, 0, False))
        prepare_reduced.grid(row = 4, column = 0, 
            padx = 10, pady = 10)
...
def continueConversion(phrase, continue_count, stop_conversion) :
            ...
            if (not end_of_bruh) :
                ...
                print_reduced = tk.Label(pane, text = reduced_bruh, 
                    bg = general_label_bg, fg = 'black', 
                    anchor = tk.E, bd = 5, 
                    borderwidth = 2)
                ...
                if (reduced_bruh == 'Cannot be BRUH-ified further.') :
                    ...
                    duplicate_prepare_reduced.grid(row = 4 + (continue_count + 1), column = 0, 
                        padx = 10, pady = 10)
            # Give Options to Exit Program, Reset Converter, or Snake Game
                ...
                exitButton = tk.Button(pane, text = 'Exit Program', 
                    bg = main_button_bg, fg = 'white',
                    bd = 5, borderwidth = 5, 
                    command = root.quit)
                exitButton.grid(row = 4 + (continue_count + 1), column = 0, 
                    padx = 10, pady = 10)

                resetButton = tk.Button(pane, text = 'Reset Converter',                     
                    bg = main_button_bg, fg = 'white',
                    bd = 5, borderwidth = 5, 
                    command = resetScreen)
                resetButton.grid(row = 4 + (continue_count + 1), column = 2, 
                    padx = 10, pady = 10)
...
startButton = tk.Button(pane, text = 'START BRUH', 
    bg = main_button_bg, fg = 'white', 
    bd = 5, borderwidth = 5, 
    command = main)
